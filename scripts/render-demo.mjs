...
for (const key of ['postgres', 'rabbitmq', 'operations-console', 'keycloak', 'legacy-core', 'equipment-adapter', 'execution-service', 'returns-service', 'collector', 'prometheus', 'grafana', 'tempo']) if (!images[key]) throw new Error(`Load the ${key} image before rendering.`);
...
objects.push(config('proxy-config', apps, { 'default.conf': read('infra/compose/proxy.conf').replace('http://keycloak:8080', `http://keycloak.${platform}.svc.cluster.local:8080`)
  .replace('    location ~ ^/api/v1/sites/[^/]+/(equipment|commands|zones|migrations|audit)', '    location ~ ^/api/v1/sites/[^/]+/audit/execution$ { proxy_pass http://execution-service:8080; }\n    location ~ ^/api/v1/sites/[^/]+/audit/returns$ { proxy_pass http://returns-service:8080; }\n    location ~ ^/api/v1/sites/[^/]+/(equipment|commands|zones|migrations|audit)')
...
group('migrations', [migrationJob('legacy-core', 'core', images['legacy-core'], 'legacy-core', legacyTarget), migrationJob('equipment-adapter', 'adapter', images['equipment-adapter']), migrationJob('execution-service', 'execution', images['execution-service']), migrationJob('shadow-scheduler', 'shadow', images['execution-service'], 'execution-service'), migrationJob('returns-service', 'returns', images['returns-service'])]);
...
console.log(`Rendered ${objects.length} scoped Kubernetes resources and six migration Jobs under ignored local storage.`);
