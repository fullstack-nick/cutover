...
public final class BrokerProbeMain {
    ...
    public static void main(String[] ignored) {
        ...
        try {
            ...
            if(!Set.of("cutover.legacy-core.v1","cutover.equipment-adapter.v1","cutover.execution-service.v1","cutover.returns-service.v1","cutover.shadow-scheduler.v1").contains(exchange)
            ...
            try(var connection=factory.newConnection("cutover-explicit-probe");var channel=connection.createChannel()) {
                ...
                channel.basicPublish(exchange,key,true,new AMQP.BasicProperties.Builder().messageId(id.toString()).contentType("application/json").deliveryMode(2).build(),body);
                channel.waitForConfirmsOrDie(5000);
                System.out.println(JsonSupport.write(Map.of("messageId",id,"result",returned.get()?"RETURNED":"CONFIRMED")));
