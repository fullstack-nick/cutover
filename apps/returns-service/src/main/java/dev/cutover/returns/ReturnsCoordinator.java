...
public final class ReturnsCoordinator {
    ...
    private JsonNode taskRows(String site,UUID taskId){return Database.json(database,"SELECT COALESCE(jsonb_agg(jsonb_build_object('id',task_id,'movementId',movement_id,'allocationId',allocation_id,'receiptId',receipt_id,'classification',classification,'zoneId','returns','state',state,'owner','returns-service','epoch',epoch,'version',version,'lastError',last_error,'transportFailures',transport_failures,'transportPaused',transport_paused,'dispatchAcceptedAt',dispatch_accepted_at) ORDER BY (state IN ('COMPLETED','CANCELLED')),CASE WHEN state IN ('COMPLETED','CANCELLED') THEN created_at END DESC,created_at,movement_id),'[]'::jsonb) FROM (SELECT t.*,m.receipt_id,m.classification FROM return_tasks t JOIN return_movements m USING(movement_id) WHERE t.site_id=? AND (?::uuid IS NULL OR t.task_id=?::uuid) ORDER BY (t.state IN ('COMPLETED','CANCELLED')),CASE WHEN t.state IN ('COMPLETED','CANCELLED') THEN t.created_at END DESC,t.created_at,t.movement_id LIMIT 100) tasks",site,taskId,taskId);}
