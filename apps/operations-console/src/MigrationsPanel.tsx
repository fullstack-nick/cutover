...
export function MigrationsPanel({ site }: { site: string }) {
  ...
  return <div className="migrations-view">
    ...
    {sessions?.items.map(session => <article className="panel migration-session" key={session.sessionId} id={`migration-${session.sessionId}`}>
      ...
      {session.observation && (session.observation.timingBasis === 'SIMULATOR_ACCEPTANCE_UPPER_BOUND'
        ? <p className={session.observation.withinTwoSeconds ? 'migration-success' : 'notice'}>{session.observation.sampleCount} completed sample movements · dispatch confirmed within {session.observation.dispatchP99Millis} ms (sample p99) · {session.observation.withinTwoSeconds ? 'within target' : 'target missed; original sample retained'}</p>
        : <p className="notice">{session.observation.sampleCount} completed sample movements · historical timing value {session.observation.dispatchP99Millis} ms · durable dispatch timing was not verified for this retained sample.</p>)}
