...
public final class AdapterHttpClient implements DispatchPort {
    ...
    @Override public JsonNode context(String site,String zone,java.util.List<UUID> movements){return checked(http.request("POST",prefix(site)+"/zones/"+zone+"/scheduling-context",JsonSupport.MAPPER.valueToTree(Map.of("movementIds",movements))),false);}
