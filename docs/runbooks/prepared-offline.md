...
Chromium's mutable `debug.log` and `chrome_debug.log` diagnostics are excluded from browser integrity hashes. Installed executables and runtime assets remain covered, so ordinary browser use does not invalidate an otherwise prepared cache.
