...
If the history remains incomplete, leave the command blocked with its evidence visible. The local lab has no force-complete or new-identity escape path. The console's Recovery view provides the same versioned investigation workflow, including evidence review, a required reason, stale-version conflicts and the recorded audit. `node tools/scenario-driver/work-console-smoke.mjs` exercises it with real operator/supervisor sessions and independent physical/business assertions.
