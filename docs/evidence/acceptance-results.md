...
| A50 | **Failed.** The [latest full run](healthy-load.md), `load-1788921802696` on `beeabc1` with bounded execution observation turns and the domain/transport tracing profile, included all 3,000 measured movements: 89.6% within two seconds, original-eligibility p99 11,364.91 ms. All 1,980 requests returned 202, all 3,300 total effects completed once and all six databases recorded zero deadlocks. Default durable settings and the five owner/shadow pod identities and tracing settings remained unchanged. All 67 resource samples and earlier failures are retained. The scheduler regression checks pass, but the unchanged at-least-99% sustained target remains unmet. |
