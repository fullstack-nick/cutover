...
| A48 | [Current prepared offline rehearsal](prepared-offline-2026-09-09.md), `offline-1788919092593`: actual login, both products, lost-response recovery, simulator and whole-lab restarts, cached predecessor import/rollback, telemetry and inspected populated dashboards under scoped denial; exact firewall cleanup verified. The current domain/transport tracing configuration and original application images are restored after rollback; unrelated container identities and states are preserved. |
| A49 | `cache-1788919055444`: all 4,050 installed assets match the current `c56cff9` images, excluding mutable Chromium diagnostic logs. An isolated empty-file-cache fixture fails explicitly and identifies all missing tools, Maven/npm packages, image archives and browser revisions; existing caches/daemon images remain intact. |
| A50 | **Failed.** The [latest full run](healthy-load.md), `load-1788918305739` on `c56cff9` with the domain/transport tracing profile, included all 3,000 measured movements: 88.3% within two seconds, original-eligibility p99 9,611.997 ms. All 3,300 total effects completed once and all six databases recorded zero deadlocks. It followed one explicitly approved clean-cache reclamation; all 46 container states, 19 Cutover pod identities, warehouse state, routes and durable settings were preserved during preparation. Every container and all five owner/shadow pod identities/settings remained unchanged through the workload. All 67 resource samples and earlier failures are retained. Improved host memory headroom did not resolve the unchanged at-least-99% target. |
