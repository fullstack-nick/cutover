...
| A50 | **Failed.** The [latest full run](healthy-load.md), `load-1788921802696` on `beeabc1`, included all 3,000 measured movements: 89.6% within two seconds, original-eligibility p99 11,364.91 ms to the journal's pre-commit timestamp. This understates durable acceptance latency; the failure remains valid. All 1,980 requests returned 202, all 3,300 total effects completed once and all six databases recorded zero deadlocks. Default durable settings and the five owner/shadow pod identities and tracing settings remained unchanged. All 67 resource samples and earlier failures are retained. The [corrected post-transaction verifier](../adr/0030-prove-dispatch-timing-after-commit.md) reproduced a complete later two-minute diagnostic: 92.833% / p99 7,189 ms. The unchanged at-least-99% sustained target remains unmet. |
