...
| A48 | [Current prepared offline rehearsal](prepared-offline-2026-09-09.md), `offline-1788910570635`: actual login, both products, lost-response recovery, simulator and whole-lab restarts, cached predecessor import/rollback, telemetry and inspected populated dashboards under scoped denial; exact firewall cleanup verified. |
| A49 | `cache-1788910462421`: all 4,050 installed assets match the current `9dc5363` images, excluding mutable Chromium diagnostic logs. An isolated empty-file-cache fixture fails explicitly and identifies all missing tools, Maven/npm packages, image archives and browser revisions; existing caches/daemon images remain intact. |
| A50 | **Failed.** The [latest full run](healthy-load.md), `load-1788916732445` on `c56cff9` with the domain/transport tracing profile, included all 3,000 measured movements: 93.4% within two seconds, original-eligibility p99 6,988.642 ms. All 3,300 total effects eventually completed once and all six databases recorded zero deadlocks. Default durable-commit settings and the five owner/shadow pod identities/tracing settings were unchanged. The report includes all 67 resource samples and retained earlier failures. The at-least-99% target and complete denominator remain unchanged. |
