...
**Current status: implementation in progress.** The latest completed full backend run passed 105 component checks, including 1,000 persisted SQL/Java scheduling comparisons and durable migration proof. Real process checks demonstrate a zone switch, stale-owner fencing, concurrent supervisors and business reversal alongside the existing delivery and equipment-recovery checks. Migration latency failures remain visible in the [selected evidence](docs/evidence/zone-migration-2026-09-08.md). Returns, full recovery, prepared offline operation and the final acceptance suite remain in development. Follow the [implementation ledger](docs/planning/implementation-progress.md) for evidence and limits.
