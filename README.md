...
The full offline backend verification passed **197 checks** across 25 test classes, with zero failures, errors or skips, in **9 minutes 58 seconds**. It covers shared messaging/security, every product-owner workflow, fair work selection, independent adapter scheduling, storage pressure, bounded inbox commits and subscriptions, database/authentication error responses and [N/N−1 contract consumers](contracts/compatibility/README.md). [Per-class results](docs/evidence/backend-checks.json) remain separate from platform acceptance.
...
The latest full load qualification on the `b3b09db` runtime offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements were included: **91.733% reached durable adapter acceptance within two seconds; p99 was 11,988.404 ms**. All 3,300 total movements eventually completed once, with zero database deadlocks. A temporary group-commit experiment did not meet the target and its original settings were restored. The target stays at least 99% within two seconds, measured from original movement eligibility. Captured storage stalls and expired batch snapshots remain under investigation; sustained-load qualification is still open.
