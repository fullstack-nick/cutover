...
**Current status: implementation in progress.** The latest full backend run passed 88 component checks, including 1,000 persisted SQL/Java scheduling comparisons, guarded task-boundary migration and independent execution. Real process checks cover the legacy workflow, durable broker delivery, process crashes, equipment uncertainty, cancellation, storage gates and audited message repair. The local kind/Calico platform runs an authenticated console with working metrics/traces. Live ownership migration, returns, full recovery and the final acceptance suite remain in development. Follow the [implementation ledger](docs/planning/implementation-progress.md) for evidence and limits.
