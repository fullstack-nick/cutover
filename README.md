...
The [latest full load qualification](docs/evidence/healthy-load.md) used the `c56cff9` runtime with the [domain/transport tracing profile](docs/adr/0027-domain-and-transport-tracing-profile.md). It offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements were included: **93.4% reached durable adapter acceptance within two seconds; p99 was 6,988.642 ms**. All 3,300 total movements eventually completed once, with zero database deadlocks and default durable-commit settings. The target stays at least 99% within two seconds, measured from original movement eligibility. The report includes resource use, backlog ages and retained earlier failures; sustained-load qualification is still open.
