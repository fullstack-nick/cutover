...
The [latest full load qualification](docs/evidence/healthy-load.md) used the `dd13096` runtime with shared route-authority reads and the [domain/transport tracing profile](docs/adr/0027-domain-and-transport-tracing-profile.md). It offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements had complete post-transaction timing proof: **93.967% within two seconds; p99 6,936 ms**. All 3,300 total movements completed once, with zero database deadlocks and default durable-commit settings. The at-least-99% target remains unmet. The [complete report](docs/evidence/healthy-load.md) preserves the failed result, resource observations and earlier attempts.
