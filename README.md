...
**Current status: implementation in progress.** The latest full backend run passed 59 component checks. Real process checks cover the legacy workflow, durable broker delivery, process crashes, equipment uncertainty, cancellation, storage gates and audited message repair. The local kind/Calico platform runs an authenticated console with 44 verified network paths and working metrics/traces. Extraction, migration, returns, full recovery and the final acceptance suite remain in development. Follow the [implementation ledger](docs/planning/implementation-progress.md) for evidence and limits.
