...
The full offline backend verification passed **204 checks** across 25 test classes, with zero failures, errors or skips, in **8 minutes 54 seconds**. It covers shared messaging/security, every product-owner workflow, fair work selection, independent adapter scheduling, refreshed batch evidence, isolated and bounded command observations, shared command and allocation authority reads with owner fencing, storage pressure, bounded inbox commits and subscriptions, database/authentication error responses and [N/N−1 contract consumers](contracts/compatibility/README.md). [Per-class results](docs/evidence/backend-checks.json) remain separate from platform acceptance.
...
The [latest full load qualification](docs/evidence/healthy-load.md) used the `beeabc1` runtime with bounded execution observation turns and the [domain/transport tracing profile](docs/adr/0027-domain-and-transport-tracing-profile.md). It offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements were included: **89.6% reached the journal row's creation timestamp within two seconds; p99 was 11,364.91 ms**. This timestamp precedes commit, so it understates durable acceptance latency; the failure remains valid. All 3,300 total movements eventually completed once, with zero database deadlocks and default durable-commit settings. The target stays at least 99% within two seconds, measured from original movement eligibility. The [corrected verifier](docs/adr/0030-prove-dispatch-timing-after-commit.md) now requires complete post-transaction timing proof. A [later two-minute diagnostic](docs/evidence/shared-authority-diagnostic.md) on the `6cbbccc` runtime also failed: 92.833% within two seconds, p99 7,189 ms after transaction return. Sustained-load qualification is still open; the reports retain resource measurements and earlier failures.
