...
The [latest full load qualification](docs/evidence/healthy-load.md) used the `beeabc1` runtime with bounded execution observation turns and the [domain/transport tracing profile](docs/adr/0027-domain-and-transport-tracing-profile.md). It offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements were included: **89.6% reached durable adapter acceptance within two seconds; p99 was 11,364.91 ms**. All 3,300 total movements eventually completed once, with zero database deadlocks and default durable-commit settings. The target stays at least 99% within two seconds, measured from original movement eligibility. The report includes resource use, backlog ages and retained earlier failures, including the earlier approved host-cache experiment; sustained-load qualification is still open.
