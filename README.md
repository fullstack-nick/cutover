...
The full offline backend verification passed **202 checks** across 25 test classes, with zero failures, errors or skips, in **9 minutes 1 second**. It covers shared messaging/security, every product-owner workflow, fair work selection, independent adapter scheduling, refreshed batch evidence, isolated and bounded command observations, storage pressure, bounded inbox commits and subscriptions, database/authentication error responses and [N/N−1 contract consumers](contracts/compatibility/README.md). [Per-class results](docs/evidence/backend-checks.json) remain separate from platform acceptance.
...
The [latest full load qualification](docs/evidence/healthy-load.md) used the `c56cff9` runtime with the [domain/transport tracing profile](docs/adr/0027-domain-and-transport-tracing-profile.md). It offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements were included: **88.3% reached durable adapter acceptance within two seconds; p99 was 9,611.997 ms**. All 3,300 total movements eventually completed once, with zero database deadlocks and default durable-commit settings. This run followed one approved host-cache experiment, which did not resolve the failure. The target stays at least 99% within two seconds, measured from original movement eligibility. The report includes host preparation, resource use, backlog ages and retained earlier failures; sustained-load qualification is still open.
