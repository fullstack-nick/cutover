...
The [latest full load qualification](docs/evidence/healthy-load.md) on the `c56cff9` runtime offered two two-line orders and one receipt per second for ten minutes after 60 seconds of warm-up. All 3,000 measured movements were included: **88.2% reached durable adapter acceptance within two seconds; p99 was 11,551.883 ms**. All 3,300 total movements eventually completed once, with zero database deadlocks and default durable-commit settings. The target stays at least 99% within two seconds, measured from original movement eligibility. The report includes resource use, backlog ages and retained earlier failures; sustained-load qualification is still open.
