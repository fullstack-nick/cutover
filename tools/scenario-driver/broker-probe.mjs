...
export async function publishFrom(service, event, { exchange = `cutover.${service}.v1`, messageId = event.eventId, rawBody, repeat = 1 } = {}) {
  assert.ok(['legacy-core', 'equipment-adapter', 'execution-service', 'returns-service'].includes(service));
  assert.ok(Number.isInteger(repeat) && repeat >= 1 && repeat <= 10001);
  ...
  const body = { exchange, routingKey: event.eventType, messageId, repeat, bodyBase64: Buffer.from(rawBody ?? JSON.stringify(event)).toString('base64') };
  return new Promise((done, failed) => {
    ...
    const timeout = repeat === 1 ? 30000 : 180000;
    const timer = setTimeout(() => { child.kill(); failed(new Error(`The owned broker probe exceeded its ${timeout / 1000}-second process limit.`)); }, timeout);
    child.on('error', failed);
    child.on('exit', code => {
      clearTimeout(timer);
      const result = stdout.split(/\r?\n/).filter(line => line.startsWith('{')).map(line => { try { return JSON.parse(line); } catch { return null; } }).find(value => ['CONFIRMED', 'RETURNED', 'PUBLISH_FAILED', 'NACKED'].includes(value?.result));
